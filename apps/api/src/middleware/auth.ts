...
export function authorize(...allowedRoles: string[]) { return (req: AuthRequest, _res: Response, next: NextFunction) => {
  const hasAccess = !!req.auth && req.auth.roles.some((role) => allowedRoles.includes(role) || (role === 'ADMIN' && allowedRoles.includes('ADMINISTRATOR')));
  return hasAccess ? next() : next(new AppError(403, 'FORBIDDEN', 'Insufficient permission'));
}; }
