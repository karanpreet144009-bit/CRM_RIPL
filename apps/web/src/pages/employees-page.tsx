  ...
  return <section>
    ...
    {open && <form onSubmit={(event) => { event.preventDefault(); setNotice(null); create.mutate(form); }} className="mt-6 rounded-lg border bg-white p-6">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm font-medium text-slate-700">Role<select value={form.role} onChange={(event) => update('role', event.target.value)} className="mt-1 w-full rounded border border-slate-300 p-2.5"><option value="SALES_EXECUTIVE">Sales Executive</option><option value="MANAGER">Manager</option><option value="ADMIN">Admin</option><option value="RECEPTION">Reception</option><option value="ACCOUNTANT">Accounts</option><option value="LICENSING_OFFICER">Licensing Officer</option></select></label>
      </div>
